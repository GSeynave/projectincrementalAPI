<template>
    ...
    <p v-if="user && user.personnages">Votre personnage <b>{{ user.personnages[0].nom }}</b> est actuellement dans la zone <b>{{ user.personnages[0].nomZone}}</b></p>

    <ul v-if="zones">

        <li v-for="(zone) in zones" :key="zone.id">
            <button type="button" @click="$emit('setPersonnageZone', zone)">{{ zone.nom }}</button>
